...
1. `Object`构造函数：
    ...
    12. `Object.assign(目标对象, 多个源对象)`
        ...
        1. 描述：将所有自有（不在原型链上）可枚举属性从多个源对象浅复制到目标对象。
        2. 返回：修改的目标对象。
...
13. Promise对象
    ...
    - 特性
        ...
        2. 在Promise对象内部抛出错误被Promise对象获取，则直接reject，值为错误信息，函数体内不再继续向下执行。
        3. 非Promise对象当作resolve。
        4. Promise对象内部的错误仅会导致reject，不会向上抛出错误。
        5. Promise对象被reject（`reject()`或函数体内抛出错误）却没有被`catch`或`then`处理，并且不在`await`调用的方法体内进行，则会触发`window`的`unhandledrejection`事件，错误不会向上抛出（不会交给`window`的`error`处理）。
...
21. `super`
    ...
    指向当前对象的原型对象（只能用在对象的方法中，目前只有对象的简写方法允许使用）。
22. `Reflect`
...
1. `async-await`异步函数
    ...
    a() // async方法返回的是Promise对象；若方法体内抛出错误则是reject，否则是resolve（包括默认返回的undefined）；返回值或抛出异常值为向下传递的值
    ```
    </details>
2. 共享内存和原子
3. `Object.entries()`、`Object.values()`
4. `String.prototype.padStart`、`String.prototype.padEnd`字符串填充
